...
//)
//
//// TestCronJob_Execution_ExecuteFuncFailure verifies that the CronJob correctly handles
//// failures within the ExecuteFunc, updating the job's status accordingly.
...
//	mockPipeline := mocks.NewMockPipeliner(ctrl)  // First pipeline (Execution)
//	mockPipeline2 := mocks.NewMockPipeliner(ctrl) // Second pipeline (Stop)
//
...
//
//	// ----------------------------
//	// Mock Pipeline.Exec Call for mockPipeline (Execution) - Simulate failure
//	// ----------------------------
//	// Since ExecuteFunc will return an error, we can choose to have Exec still succeed
//	// to ensure state updates are attempted. Alternatively, to simulate a failure in state update:
//	// Uncomment the following lines to simulate Exec failure
//	/*
//		mockPipeline.EXPECT().Exec(gomock.Any()).
//			Return([]redis.Cmder{cmdSet1, cmdZAdd1}, fmt.Errorf("pipeline execution failed")).
//			Times(1)
//	*/
//	// For this test, we'll assume Exec succeeds but ExecuteFunc fails
...
//
//	// ----------------------------
//	// Mock Pipeline.Exec Call for mockPipeline2 (Stop)
//	// ----------------------------
...
//	}
//
//	// Verify that Iterations is incremented despite the failure
...
//	}
//
//	// Verify that Status is updated to "Failed" due to ExecuteFunc failure
//	if state.Status != cron.JobStatusFailed {
//		t.Fatalf("expected Status to be 'Failed', got '%s'", state.Status)
//	}
//
